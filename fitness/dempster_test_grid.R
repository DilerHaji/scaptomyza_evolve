# Dempster protection under arithmetic-mean overdominance:
#   W_Aa > max(W_AA, W_aa), where W_G = sum_i c_i * w_G,i.

# Equilibrium ( overdominance ):
#   q* = (W_Aa - W_aa) / (2*W_Aa - W_AA - W_aa).

# With the same symmetric-antagonistic parameterization used in the Levene
# grid (w1_AA = 1+2s1, w1_Aa = 1+s1, w1_aa = 1; w2_AA = 1, w2_Aa = 1+s2, w2_aa = 1+2s2)

# inequalities:
#   c*s1 > (1-c)*s2  AND  (1-c)*s2 > c*s1
...
# main model (for G×E significance)
...
p_label <- ifelse(p_val_interaction < 0.001, "p < 0.001",
                  paste0("p = ", round(p_val_interaction, 3)))

# bootstrap for(c, k) cloud
set.seed(123)
n_boot <- 1000
...
for (i in 1:n_boot) {
  ...
  c_design <- 0.5
  W_AA <- c_design * w_B_AA + (1 - c_design) * w_T_AA
  W_Aa <- c_design * w_B_Aa + (1 - c_design) * w_T_Aa
  W_aa <- c_design * w_B_aa + (1 - c_design) * w_T_aa
  dem_stable <- (W_Aa > W_AA) & (W_Aa > W_aa)
  denom <- 2 * W_Aa - W_AA - W_aa
  q_dem <- if (dem_stable && abs(denom) > 1e-9) (W_Aa - W_aa) / denom else NA_real_

  results_list[[i]] <- data.frame(Iteration = i,
                                  c = val_c, k = k_val,
                                  dempster_stable = dem_stable,
                                  q_dempster = q_dem)
...
p_dem_empirical <- mean(sim_data$dempster_stable, na.rm = TRUE)
message(sprintf("\nEmpirical Dempster stability rate (c = 0.5): %.1f%%",
                100 * p_dem_empirical))
...
calculate_dempster_q <- function(c, s_base, k_asymmetry) {
  ...
  if (invade_A && invade_a) {
    denom <- 2 * W_Aa - W_AA - W_aa
    if (abs(denom) < 1e-9) return(0.5)
    return((W_Aa - W_aa) / denom)
  } else {
    if (!invade_A) return(0.0)   # a fixes
    if (!invade_a) return(1.0)   # A fixes
  }
  NA
}
...
for (s_val in s_levels) {
  ...
  temp_grid$q_hat   <- mapply(calculate_dempster_q,
                              temp_grid$Availability_c, s_val, temp_grid$Selection_k)
  temp_grid$s_facet <- paste0("s = ", s_val)
  grid_list[[as.character(s_val)]] <- temp_grid

  # stability check on bootstrap cloud for this background s
  s1_vec <- rep(s_val, nrow(sim_data))
  s2_vec <- pmin(s_val * sim_data$k, 10)

  W_AA_vec <- sim_data$c * (1 + 2 * s1_vec) + (1 - sim_data$c) * 1
  W_Aa_vec <- sim_data$c * (1 +     s1_vec) + (1 - sim_data$c) * (1 +     s2_vec)
  W_aa_vec <- sim_data$c * 1               + (1 - sim_data$c) * (1 + 2 * s2_vec)

  invade_A <- W_Aa_vec > W_aa_vec
  invade_a <- W_Aa_vec > W_AA_vec
...
# plot
p_grid <- ggplot() +
  geom_tile(data = all_grids,
            aes(x = Availability_c, y = Selection_k, fill = q_hat)) +
  scale_fill_gradient2(low = "#EDB72D", mid = "#9BAB96", high = "#499FFF",
                       midpoint = 0.5, limits = c(0, 1),
                       name = expression(hat(q)),
                       na.value = "grey85") +
  geom_contour(data = all_grids,
               aes(x = Availability_c, y = Selection_k, z = q_hat),
               breaks = c(0.01, 0.99),
  ...
  coord_cartesian(ylim = c(0, 8), xlim = c(0, 1), expand = FALSE) +
  labs(title = "Dempster hard-selection stability (parameterized)",
       subtitle = paste0("Symmetric antagonistic fitness model; dashed contours mark fixation boundary. ",
                         "Empirical bootstrap Dempster-stability at c = 0.5: ",
                         round(100 * p_dem_empirical, 1), "%"),
...
print(p_grid)
ggsave("Dempster_Grid_Annotated.png", p_grid, width = 8, height = 4, scale = 1.5)
ggsave("Dempster_Grid_Annotated.svg", p_grid, width = 8, height = 4, scale = 1.5)